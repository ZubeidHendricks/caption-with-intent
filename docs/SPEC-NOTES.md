...
**Word spacing — absent entirely.** The spec fixes size, weight and width per word but says nothing about the gap between words, and a variable font's natural space advance is calibrated for regular weight. At `wght` 900 / `wdth` 122 — which the system asks for whenever a low, resonant voice speaks — adjacent words visibly weld together, and obliqued off-camera text is worse because the lean carries the last glyph into the space. `wordGap()` in `@chorus/core` scales the gap with weight and width, plus 15% when slanted. This is a real omission in V1.0, not a rendering detail.
