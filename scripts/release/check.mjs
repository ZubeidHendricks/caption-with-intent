...
for (const name of NAMES) {
  ...
  for (const [bin, target] of Object.entries(json.bin ?? {})) {
    // npm reports a leading "./" as an error it "removed" during publish, then
    // quietly auto-corrects it in the registry metadata — 0.1.0 shipped this
    // way and installs correctly. So this is hygiene, not a defect: depending
    // on an undocumented auto-correction is a poor bet, and writing the path
    // the way npm wants costs nothing. A note rather than a failure, because
    // blocking a release over something demonstrably working is worse.
    if (target.startsWith('./')) {
      notes.push(`  ${name}: bin "${bin}" -> "${target}" — npm rewrites this to ` +
        `"${target.slice(2)}" at publish; write it that way and stop relying on that`);
    }
