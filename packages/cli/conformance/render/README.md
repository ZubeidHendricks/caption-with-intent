...
```
chorus conform-render --report my-report.json --scene dialogue-2speaker
```
...
Sample at exactly the times the scene lists — `chorus render-scenes` prints them,
...
`chorus render-scenes` lists them with their sample times. Each is a complete
